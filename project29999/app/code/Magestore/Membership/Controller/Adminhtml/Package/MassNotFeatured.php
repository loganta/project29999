 ...
 */
class MassNotFeatured extends \Magestore\Membership\Controller\Adminhtml\Package
{
        ...
        if (!is_array($entityIds) || empty($entityIds)) {
            ...
            try {
                ...
                foreach ($collection as $item) {
                    $item->setIsFeatured(FeaturedOptions::NOT_FEATURED)
                        ->setIsMassupdate(true)
                        ->save();
                }
                $this->messageManager->addSuccess(
                    __('%1 package(s) turned into normal package.', count($entityIds))
                );
            } catch (\Exception $e) {
