 ...
 */
class MassEnable extends \Magestore\Membership\Controller\Adminhtml\Package
{
        ...
        if (!is_array($entityIds) || empty($entityIds)) {
            ...
            try {
                ...
                foreach ($collection as $item) {
                    $item->setPackageStatus(Status::STATUS_ENABLED)
                        ->setIsMassupdate(true)
                        ->save();
                }
                $this->messageManager->addSuccess(
                    __('A total of %1 package(s) have been enabled.', count($entityIds))
                );
            } catch (\Exception $e) {
