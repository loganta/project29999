 ...
 */
class MassDisable extends \Magestore\Membership\Controller\Adminhtml\Member
{
        ...
        if (!is_array($entityIds) || empty($entityIds)) {
            ...
            try {
                ...
                foreach ($collection as $item) {
                    $item->setMemberStatus(Status::STATUS_DISABLED)
                ...
                $this->messageManager->addSuccess(
                    __('A total of %1 member(s) have been disabled.', count($entityIds))
                );
            } catch (\Exception $e) {
