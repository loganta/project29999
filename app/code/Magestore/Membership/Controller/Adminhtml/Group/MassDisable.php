 ...
 */
class MassDisable extends \Magestore\Membership\Controller\Adminhtml\Group
{
        ...
        if (!is_array($entityIds) || empty($entityIds)) {
            ...
            try {
                ...
                foreach ($collection as $item) {
                    $item->setGroupStatus(Status::STATUS_DISABLED)
                        ->setIsMassupdate(true)
                        ->save();
                }
                $this->messageManager->addSuccess(
                    __('A total of %1 product group(s) have been disabled.', count($entityIds))
                );
            } catch (\Exception $e) {
